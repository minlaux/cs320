...
let intrep_add(ds1: string)(ds2: string): string =
    ...
    let rec add_strings(i: int)(j: int)(carry: int)(result: string): string =
        if i < 0 && j < 0 then
            if carry > 0 then
                let carry_char = char_of_digit carry in
                string_cons (carry_char)(result)
            ...
            add_strings (i - 1) (j - 1) (new_carry) (string_cons (result_char)(result))
